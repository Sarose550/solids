...
def scanline_convert(polygons, i, screen, zbuffer ):
    #determine the traingle
    pointslist = [polygons[i + k][:] for k in range(3)]
    #find top, middle, bottom
    pointslist.sort(key=my_key)
    #print(pointslist)
    xb = pointslist[0][0]
    yb = pointslist[0][1]
    zb = pointslist[0][2]
    ...
    dx0 = (xt - xb) / (yt - yb + 1)
    dz0 = (zt - zb) / (yt - yb + 1)
    if(abs(ym - yb) >= epsilon):
        dx1 = (xm - xb) / (ym - yb + 1)
        dz1 = (xm - xb) / (zm - zb + 1)
    else:
        dx1 = 0
        dz1 = 0
    if(abs(yt - ym) >= epsilon):
        dx1_flip = (xt - xm) / (yt - ym + 1)
        dz1_flip = (zt - zm) / (yt - ym + 1)
    else:
        dx1_flip = 0
        dz1_flip = 0
    color = [random.randrange(255) for k in range(3)]
    #draw the scanlines
    while(y <= yt):
...
def draw_scanline(screen, zbuffer, color, x0, x1, y, z0, z1):
    if(x0 > x1):
        draw_scanline(screen, zbuffer, color, x1, x0, y, z1, z0)
        return
    if(x0 == x1):
        return
    else:
        #if(abs(x1 - x0) > 200):
        #    print("some shit's gone wrong")
        #    print(x0, y, z0)
        #    print(x1, y, z1)
        x = x0
        z = z0
        z_slope = (z1 - z0) / (x1 - x0 + 1)
        while(x <= x1):
